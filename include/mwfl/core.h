...
template <typename ClosePolicy> class UniqueHandle final {
public:
    ...
    [[nodiscard]] handle_type Release() noexcept { return std::exchange(handle_, ClosePolicy::Invalid()); }
    void Reset(handle_type replacement = ClosePolicy::Invalid()) noexcept { if (ClosePolicy::IsValid(handle_)) ClosePolicy::Close(handle_); handle_ = replacement; }
private:
...
struct KernelHandlePolicy {
    ...
    [[nodiscard]] static bool IsValid(HANDLE handle) noexcept { return handle != nullptr && handle != INVALID_HANDLE_VALUE; }
...
struct WaitResult { WaitStatus status = WaitStatus::Timeout; };
[[nodiscard]] Result<WaitResult> WaitForHandle(HANDLE handle, std::chrono::milliseconds timeout, std::stop_token stop = {});
