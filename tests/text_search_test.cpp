...
int main() {
    ...
    if (mwfl::ReplaceAllText(replace, L"cat", L"dog", {.whole_word = true}) != 3 ||
        replace != L"dog catalog dog dog") return EXIT_FAILURE;
    std::wstring growth = L"a a";
    if (mwfl::ReplaceAllText(growth, L"a", L"aa") != 2 || growth != L"aa aa")
        return EXIT_FAILURE;
    std::wstring shrink = L"aaaa";
    if (mwfl::ReplaceAllText(shrink, L"aa", L"a") != 2 || shrink != L"aa")
        return EXIT_FAILURE;
    return EXIT_SUCCESS;
}
