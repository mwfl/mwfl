...
int main() {
    ...
    if (ParseDocumentSession(L"MWTL_DOCUMENT_SESSION 2 0\n").status !=
    ...
    const auto root = std::filesystem::temp_directory_path() /
        (L"mwtl-session-" + std::to_wstring(::GetCurrentProcessId()));
    std::filesystem::create_directories(root);
    const auto path = root / L"session.mwtl";
    if (SaveDocumentSessionAtomic(path, session) != DocumentSessionStatus::success)
        return 10;
    const auto interrupted = path.wstring() + L".mwtl-tmp-interrupted";
    ...
    if (!loaded || loaded.session->workspaces[0].documents[0].metadata.id != DocumentId{11})
        return 11;
    if (!WriteTextFileAtomic(path, L"MWTL_DOCUMENT_SESSION 1 1\nW 7").Succeeded() ||
