...
int main() {
    ...
    if (store.Load(schema).status != VersionedSettingsStatus::not_found) return 1;
    ...
    if (!saved) return saved.status == VersionedSettingsStatus::access_denied ? 0 : 2;
    ...
    ::RegCloseKey(raw);
    if (store.Load(schema).status != VersionedSettingsStatus::version_mismatch) return 6;
    ...
    ::RegCloseKey(raw);
    if (store.Load(schema).status != VersionedSettingsStatus::malformed || !store.Save(values)) return 13;
    ...
    if (::RegGetValueW(HKEY_CURRENT_USER, key.path.c_str(), L"Unrelated", RRF_RT_REG_DWORD,
        ...
        return 9;
    if (store.Load(schema).status != VersionedSettingsStatus::malformed) return 10;
