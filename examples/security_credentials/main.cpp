...
int wmain(int argc, wchar_t** argv) {
    ...
    if (argc == 2 && std::wstring_view(argv[1]) == L"--integration") { (void)mwfl::CredentialManager::Remove(target); const char value[] = "temporary"; mwfl::GenericCredential credential{target, L"mwfl", mwfl::SecureBytes(std::span(reinterpret_cast<const std::byte*>(value), sizeof(value) - 1))}; auto written = mwfl::CredentialManager::Write(std::move(credential)); if (!written) { (void)mwfl::CredentialManager::Remove(target); return 1; } auto read = mwfl::CredentialManager::Read(target); auto removed = mwfl::CredentialManager::Remove(target); auto audit = mwfl::CredentialManager::Read(target); return read && removed && removed.Value() && !audit ? 0 : 2; }
    ...
    mwfl::SecureWideString secret(L"ephemeral"); if (secret.View() != L"ephemeral") return 2; secret.Clear();
