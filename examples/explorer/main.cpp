...
constexpr mwtl::ControlId kBack{700};
constexpr mwtl::ControlId kUp{701};
constexpr mwtl::ControlId kRefresh{702};
constexpr mwtl::ControlId kSortName{703};
constexpr mwtl::ControlId kOpen{710};
constexpr mwtl::ControlId kProperties{711};
...
class ExplorerWindow final : public mwtl::WindowBase {
public:
    void BuildUI() override {
        SetTitle(L"mwtl Explorer");
        ...
        ui.Add(splitter_, {203}, {0.0_dip, 76.0_dip, 980.0_dip, 540.0_dip},
               mwtl::SplitterOptions{.constraints = {180.0_dip, 320.0_dip, 6.0_dip},
                                     .initial_position = 250.0_dip});
        mwtl::ControlHost panes{splitter_};
        panes.Add(tree_, {204}, mwtl::RectDip{});
        panes.Add(list_, {205}, mwtl::RectDip{},
        ...
        mwtl::Must(toolbar_.SetImageList(images_), "attach toolbar image list");
        for (const auto& command : commands_.GetCommands()) {
            if (command.GetId() == kOpen || command.GetId() == kProperties) continue;
            mwtl::Must(toolbar_.AddCommand(command), "add Explorer toolbar command");
        }
        toolbar_.AutoSize();
        mwtl::Must(rebar_.AddBand(toolbar_, L"Navigation", 500), "add Explorer rebar band");
        mwtl::Must(accelerators_.Create(commands_), "create Explorer accelerators");
        SetAccelerators(accelerators_.GetHandle());

        mwtl::Must(mwtl::AddColumns(list_, {{L"Name", 300}, {L"Type", 210}, {L"Size", 120}}),
        ...
        PopulateTree();
        mwtl::Must(list_.SetVirtualModel(model_), "attach Explorer virtual model");

        mwtl::Must(tab_model_.Add({{1}, L"All files", false, false}), "add All files tab");
        mwtl::Must(tab_model_.Add({{2}, L"Pictures", false, false}), "add Pictures tab");
        mwtl::Must(tabs_.Synchronize(tab_model_), "synchronize Explorer tabs");

        mwtl::Must(mwtl::SetAccessibleName(toolbar_.GetHwnd(), L"Explorer commands"),
                   "name Explorer toolbar");
        mwtl::Must(mwtl::SetAccessibleName(tree_.GetHwnd(), L"Folders"),
                   "name Explorer tree");
        mwtl::Must(mwtl::SetAccessibleName(list_.GetHwnd(), L"Folder contents"),
                   "name Explorer list");
        mwtl::Must(mwtl::SetAccessibleName(tabs_.GetHwnd(), L"Explorer views"),
                   "name Explorer tabs");

        SetLayout(mwtl::Column()
                      .Add(rebar_, mwtl::Auto())
                      .Add(tabs_, mwtl::Fixed(34.0_dip))
    ...
    mwtl::EventResult OnNotify(const mwtl::NotifyEvent& event) override {
        if (event.IsFrom(list_)) {
            ...
            if (const auto notification = list_.DecodeNotification(event.header)) {
                if (notification->kind == mwtl::ListViewNotificationKind::column_clicked) {
                    SortByColumn(notification->column);
                    return mwtl::EventResult::Handled();
                }
                if (notification->kind == mwtl::ListViewNotificationKind::selection_changed) {
        ...
        if (event.IsFrom(tree_)) {
            if (const auto notification = tree_.DecodeNotification(event.header);
                notification &&
                notification->kind == mwtl::TreeViewNotificationKind::selection_changed) {
        ...
        if (event.IsFrom(tabs_) && event.header.code == TCN_SELCHANGE) {
            ...
            tree_.SetSelection(selected == mwtl::TabId{2} ? mwtl::TreeItemId{20}
                                                          : mwtl::TreeItemId{1});
            status_.SetPartText(2, selected == mwtl::TabId{2} ? L"Pictures filter"
                                                              : L"All files filter");
            return mwtl::EventResult::Handled();
        }
...
private:
    void BuildImagesAndCommands() {
        mwtl::Must(images_.Create(16, 16), "create Explorer image list");
        for (const wchar_t* icon_name : {IDI_APPLICATION, IDI_INFORMATION, IDI_ASTERISK,
                                        IDI_WARNING}) {
            mwtl::Must(images_.AddIcon(::LoadIconW(nullptr, icon_name)) >= 0,
                       "add Explorer image");
        }
        commands_.Add(mwtl::Command{kBack, L"Back", [this] { GoBack(); }}
                          .SetImageIndex(0)
                          .SetShortcut({FALT | FVIRTKEY, VK_LEFT}));
        commands_.Add(mwtl::Command{kUp, L"Up", [this] { GoUp(); }}
                          .SetImageIndex(1)
                          .SetShortcut({FALT | FVIRTKEY, VK_UP}));
        commands_.Add(mwtl::Command{kRefresh, L"Refresh", [this] { Refresh(); }}
                          .SetImageIndex(2)
                          .SetShortcut({FVIRTKEY, VK_F5}));
        commands_.Add(mwtl::Command{kSortName, L"Sort", [this] { ToggleNameSort(); }}
                          .SetImageIndex(3));
        commands_.Add(mwtl::Command{kOpen, L"Open", [this] { OpenSelection(); }});
        commands_.Add(mwtl::Command{kProperties, L"Properties", [this] { ShowProperties(); }});
    }
    ...
    void PopulateTree() {
        ...
        for (const auto& folder : folders) {
            ...
            mwtl::Must(item != nullptr, "add Explorer folder");
            TVITEMW native{};
            native.mask = TVIF_IMAGE | TVIF_SELECTEDIMAGE;
            native.hItem = item;
            native.iImage = folder.parent ? 1 : 0;
            native.iSelectedImage = native.iImage;
            mwtl::Must(TreeView_SetItem(tree_.GetHwnd(), &native) != FALSE,
                       "set Explorer folder image");
        }
        mwtl::Must(tree_.Expand({1}), "expand Explorer root");
    }

    void Navigate(mwtl::TreeItemId folder, bool record_history) {
        if (!folder || folder == model_->GetSelectedFolder()) return;
        if (record_history) history_.push_back(model_->GetSelectedFolder());
        mwtl::Must(list_.UpdateVirtualModel([&] {
                       mwtl::Must(model_->SelectFolder(folder), "select Explorer folder");
                   }),
                   "refresh Explorer folder");
        tabs_.SetSelection(folder == mwtl::TreeItemId{20} ? mwtl::TabId{2}
                                                          : mwtl::TabId{1});
    ...
    void Refresh() {
        mwtl::Must(list_.RefreshVirtualModel(), "refresh Explorer list");
    ...
    void SortByColumn(int column) {
        ...
        mwtl::Must(list_.UpdateVirtualModel([&] { model_->Sort(selected, true); }),
    ...
    void ShowProperties() {
        if (context_source_ == tree_.GetHwnd()) {
            if (!g_self_test)
                mwtl::ShowTaskDialog(GetHwnd(), L"mwtl Explorer", L"Folder properties",
        ...
        mwtl::ShowTaskDialog(GetHwnd(), L"mwtl Explorer", L"Item properties",
                             L"Stable file identity remains application-owned.");
    }

    void ShowContextMenu(HWND source, POINT position) {
        mwtl::Menu menu;
        mwtl::Must(menu.CreatePopup(), "create Explorer context menu");
        mwtl::Must(menu.AppendCommand(kOpen.value, L"Open"), "add Open context command");
        mwtl::Must(menu.AppendCommand(kProperties.value, L"Properties"),
    ...
    void RunSelfTest() {
        ...
        if (static_cast<int>(::SendMessageW(rebar_.GetHwnd(), RB_GETBANDCOUNT, 0, 0)) != 1 ||
            ...
            tabs_.GetSelectedTabId() != mwtl::TabId{1} || model_->GetRowCount() != 6 ||
        ...
        if (model_->GetSelectedFolder() != mwtl::TreeItemId{20}) {
            self_test_step_ = 41;
        ...
        self_test_step_ = 63;
        if (tabs_.GetSelectedTabId() != mwtl::TabId{2})
            throw std::runtime_error("Explorer tree did not synchronize Pictures tab");
        mwtl::Must(tabs_.SetSelection(mwtl::TabId{1}), "select Explorer All files tab");
        ...
        self_test_step_ = 64;
        if (model_->GetSelectedFolder() != mwtl::TreeItemId{1} || model_->GetRowCount() != 6)
    ...
    mwtl::ImageList images_;  // Declared first: every borrowing HWND dies first.
    ...
    mwtl::TabWorkspaceModel tab_model_;
    mwtl::Splitter splitter_;
    mwtl::TreeView tree_;
    mwtl::ListView list_;
    mwtl::StatusBar status_;
    std::vector<mwtl::TreeItemId> history_;
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int show) {
    ...
    return mwtl::RunApplication<ExplorerWindow>(
        instance, show,
        {.title = L"mwtl Explorer",
