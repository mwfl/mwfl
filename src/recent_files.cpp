...
RecentFilesLoadResult LoadRecentFilesFromRegistry(
    ...
    if (schema_type != REG_DWORD || schema_size != sizeof(schema) || schema != kSchemaVersion)
        return {.status = SettingsStatus::invalid_data, .native_error = ERROR_INVALID_DATA};
    ...
    while (offset + 1 < data.size() && data[offset] != L'\0') {
        ...
        if (end == data.end()) return {.status = SettingsStatus::invalid_data, .native_error = ERROR_INVALID_DATA};
