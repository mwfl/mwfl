...
- Store preferences below an application-owned, versioned HKCU key. The Notepad
  example uses `Software\\mwfl\\Notepad\\1` and a `REG_DWORD` named
  `AlwaysOnTop`.
...
1. Add `constexpr mwfl::ControlId kAlwaysOnTop{718};`.
...
6. Extend `mwfl.notepad_gui` or an equivalent GUI test to inspect both
