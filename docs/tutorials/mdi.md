...
`mwtl::mwtl` and never becomes global application architecture.

## 1. Build the reference

```powershell
cmake --preset vs2026-x64
cmake --build --preset vs2026-x64-debug --target mwtl_mdi_workspace
ctest --preset vs2026-x64-debug -R "^mwtl\.mdi_(model|native|workspace_gui)$"
```
...
```cmake
find_package(mwtl 0.1 CONFIG REQUIRED COMPONENTS mdi)
target_link_libraries(my_app PRIVATE mwtl::mdi)
```
...
call but must not throw through a window procedure. mwtl captures exceptions;
...
```cpp
auto routed = mwtl::RouteMdiActiveChild(model, [&](mwtl::MdiChildId id) {
